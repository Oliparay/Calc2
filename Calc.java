...
public class Calc {
    public static void main(String[] args) {
        ...
        if (action == '+') {
            if (!expression.matches("\".{1,10}\"\s\\W\s\".{1,10}\"")){
                throw new IllegalArgumentException("Invalid input format");  // проверк ана формат
            }
            printInQuotes(array[0] + array[1]);
        } else if (action == '*') {
            int parse = Integer.parseInt(array[1]); // преобразование строки в число
            String result = "";
            if (parse < 1 || parse > 10) try {
                throw new Exception("Number must be between 1 and 10");  // ошибка
            } catch (Exception e){
                throw new RuntimeException(e);
            }
            for (int i = 0; parse > i; i++) {   // сложение строки
                result += array[0];
            }
            if (result.length()>40){
                printInQuotes(result.substring(0, 40) + "...");  // заменяем на ... если больше 40
            } else printInQuotes(result);
        } else if (action == '-') {
            if (!expression.matches("\".{1,10}\"\s\\W\s\".{1,10}\"")){
                throw new IllegalArgumentException("Invalid input format");
            }
            int index = array[0].indexOf(array[1]); // вычетает из первой строки вторую
            if (index == -1) {
    ...
    static void printInQuotes(String text) {  // в ответе к результату добаляет кавычки
